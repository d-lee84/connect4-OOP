...
class Game {
  ...
  switchPlayers() {
    ...
    this.players[this.players.length - 1] = this.currPlayer;
...
// Start with two players as the main 
let numPlayers = 2;
...
function addNewPlayer(evt) {
  let pColors = document.querySelector("#pColors")

  let inputDiv = document.createElement('div');
  inputDiv.classList.add("col-6", "my-2");


  let newPlayerInput = document.createElement('input');

  newPlayerInput.placeholder =`Player ${++numPlayers} Color`;
  newPlayerInput.id = `p${numPlayers}Color`;
  newPlayerInput.classList.add('d-flex', 'mx-auto', 'form-control');

  inputDiv.appendChild(newPlayerInput);

  pColors.append(inputDiv);
}
